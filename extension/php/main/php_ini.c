...
int php_init_config(TSRMLS_D)
{
	...
	if (sapi_module.php_ini_path_override) {
		char *path=sapi_module.php_ini_path_override;
		/*Fix of bug:
Warning: Unknown: failed to open stream: No such file or directory in Unknown on line 0
Fatal error: Unknown: Failed opening required 'C:/каталог/app/html/test.php' (include_path='.;C:\php_tmp\pear;c:\work\site\php\includes') in Unknown on line 0
while runing under windows from utf-8 path*/
#ifdef PHP_WIN32
	{
		int i,k;
		for (i = 0, k = 0; path[i]; k ++) {
			if (path[i] < 0) {
				path[k] = (path[i] + 48) * 64 + path[i + 1] + 48;
				i += 2;
			}
			else {
				path[k] = path[i];
...
#endif
//End of bug fix 
		php_ini_file_name = path;
		php_ini_search_path = path;
